...
function downloadFeedStatusORD(){
	...
	else
	{
	 	...
	 	$("#downloadExcel").dialog({title: 'Check Column',width: 296,height: 600});
...
function getStatusCounter()
{
	...
	success : function(data)
	{
		...
		if(data.length>0){
			...
			if(data[0].Stage1>0)
			{
			temp+='<div class="TitleBorder"><h1>Appointment</h1><div class="circle">'+data[0].Stage1+'</div></div>';
			total+=parseInt(data[0].Stage1);
			}
			if(data[0].Stage2>0)
			{
			temp+='<div class="TitleBorder"><h1>Service In</h1><div class="circle">'+data[0].Stage2+'</div></div>';
			total+=parseInt(data[0].Stage2);
			}
			if(data[0].Resolved>0)
			{
			temp+='<div class="TitleBorder"><h1>Service Out</h1><div class="circle">'+data[0].Resolved+'</div></div>';
...
{    
	    ...
	    type : "post",
	    async:false,
	    url : "view/Over2Cloud/CorporatePatientServices/cpservices/beforeCPSView.action?minDateValue="+$("#minDateValue").val()+"&maxDateValue="+$("#maxDateValue").val()+"&service_manager="+$("#service_manager").val(),
