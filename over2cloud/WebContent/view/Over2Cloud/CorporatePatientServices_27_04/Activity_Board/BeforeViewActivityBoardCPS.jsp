...
$.subscribe('getPharmacyData', function(event,data)
		{
	alert(data);
	dateClick(data);
...
function viewHistoryOnClick(id) 
{
	...
	$("#takeActionGrid").dialog({title: 'Order History for Action ',width: 1000,height: 550});
  $.ajax
	({
	type : "post",
...
function takeActionForPharmacy(cellvalue, options, rowObject) 
{
	...
	var context_path = '<%=request.getContextPath()%>';
 	if(rowObject.status=='Ordered' || rowObject.status=='Dispatch'){
	 		return "<a href='#'><img title='Take Action' src='"+ context_path +"/images/action.jpg' height='20' width='20' onClick=takeActionOnOrdered('"+rowObject.id+"','"+rowObject.status+"')> </a>";
	}
	 
	else if(rowObject.status=='Close')
	{
 		return "<a href='#'><img title='Take Request' src='"+ context_path +"/images/action.jpg' height='20' width='20' onClick=takeActionOnClosed("+rowObject.id+")> </a>";
...
function takeActionOnClosed(complainId) 
{
	...
	$("#takeActionGrid").html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>This order is already Closed!!.</div>');
	$("#takeActionGrid").dialog('open');  
 
}
 
 
function takeActionOnOrdered(complainId,status) 
{
	$("#takeActionGrid").dialog({title: 'Order Dispatch',width: 1000,height: 500});
	$("#takeActionGrid").html("<br><br><br><br><br><br><br><br><center><img src=images/ajax-loaderNew.gif></center>");
	$("#takeActionGrid").dialog('open');
	  $.ajax
	({
	type : "post",
	url  : "view/Over2Cloud/CorporatePatientServices/Pharmacy/beforeTakeActionOnOrder.action?feedStatus="+status+"&complianId="+complainId,
	success : function(data)
	{
	$("#takeActionGrid").html(data);
	},
...
function onloadDataPharmacy(flag)
{
	...
	({
		type : "post",
		url  : "view/Over2Cloud/CorporatePatientServices/Pharmacy/beforeActivityBoardDataForPharmacy.action?nursingUnit="+nursingUnit+"&priority="+priority+"&minDateValue="+minDateValue+"&maxDateValue="+maxDateValue+"&feedStatus="+status+"&escLevel="+level,
		async:false,
		success : function(data)
		{
	...
	<div style="border: blue; height: 67px;">
		...
		<div style="float: left;">
			<s:textfield id="wildSearch" name="wildSearch" cssClass="button"
				cssStyle="  width: 200px;height: 36px;margin-top: 1%;"
				Placeholder="Enter Any Value" theme="simple"
...
<%if(userRights.contains("ORDCT_VIEW") || userTpe.equalsIgnoreCase("M")) 
	...
	<s:hidden id="refreshTime" value="10"></s:hidden>
	      	...
	      	cssStyle	=	"height: 28px;margin-top: 3px;margin-left: -2px;width: 100px;"
	        	...
	        	>
									...
									<sj:a button="true" cssClass="button"
										...
										<%if(userRights.contains("ORD_Action_VIEW")) 
								{%>
								...
								<%}%>
									...
									href="#" style="height: 26px; width: 32px;"> <img
										...
										style="margin-top: 3px; margin-left: 2px; float:right; margin-right: 2px"
										title="Stat" onclick="priorityClick('Stat');"></a> <!--<sj:a  button="true" cssClass="button" cssStyle="margin-top: 0px;margin-left: 2px;height:27px; width:31px;margin-right: 2px;"  title="Download"     buttonIcon="ui-icon-arrowstop-1-s"></sj:a>
	       	      	-->
...
<div id="PharmacySearchCriteriaDiv" style="display: none">
		<div id="viewGridTable"
			...
			<div style="" class="listviewButtonLayer" id="listviewButtonLayerDiv">
						...
						<tbody>
							<tr>
								<td>
										...
										<sj:datepicker cssStyle="height: 26px; width: 93px;"
										cssClass="button" id="minDateValue1" name="minDateValue"
										size="20" maxDate="0" value="%{minDateValue}" readonly="true"
										yearRange="2013:2050" onchange="dateClick(this.value);"
										showOn="focus" displayFormat="dd-mm-yy"
										Placeholder="Select From Date" /> 
									
										<sj:datepicker
										cssStyle="height: 26px; width: 93px;" cssClass="button"
										id="maxDateValue1" name="maxDateValue" size="20"
										value="%{maxDateValue}" readonly="true" yearRange="2013:2050"
										onchange="dateClick(this.value); " showOn="focus"
										displayFormat="dd-mm-yy" Placeholder="Select To Date" /> 
										
										<s:select
										id="pharmacyStatus" name="orderStatus" 
										list="#{'Ordered':'Ordered','Dispatch':'Dispatch','Close':'Close'}"
			 							theme="simple" cssClass="button" headerKey="-1"
										headerValue="Status"
										cssStyle="height: 28px;margin-top: 3px;margin-left: -2px;width: 95px;"
										onchange="dateClick('')">
									 	...
									 	<sj:autocompleter id="nursingUnit1"
									  	name="nursingUnit"
										list="nursingUnitList" 
										selectBox="true" 
								 		selectBoxIcon="true" 
										...
										src="images/SMSPink12.png" width="20px" height="20px"
										style="margin-top: 3px; margin-left: 2px; float:right; margin-right: 2px"
										title="Urgent" onclick="priorityClick('Urgent');"></a>
										...
										src="images/SMSred12.png" width="20px" height="20px"
										style="margin-top: 3px; margin-left: 2px; float:right; margin-right: 2px"
										title="Stat" onclick="priorityClick('Stat');">
										</a>  
				...
				<div onmouseover="StopRefresh()" onmouseout="StartRefresh()"
					style="overflow: scroll; max-height: 575px;" align="center">
					<div id="viewDataDivForPharmacy"></div>
				</div>
				<!-- Code End for Header Strip -->
