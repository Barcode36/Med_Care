...
<script  src="js/jquery-ui.js"></script>
  ...
  //to get list of all department
    $.ajax({
    	    ...
    	    type : "post",
    	    url : "view/Over2Cloud/CriticalMaster/depatmentsforlist.action?deptFlag=dept",
...
function maptomultiplewing(emp){
	...
	$("#maximizeDataDiv1").html("<br><br><br><br><br><center><img src=images/lightbox-ico-loading.gif></center>");
	 $.ajax({
	    type : "post",
	    url : "/over2cloud/view/Over2Cloud/CriticalPatient/masters/mappingCritical.jsp",
	    success : function(data) 
	    {
...
var delay = (function(){
	...
	$.ajax({
	      	    type : "post",
	      	    url : "/over2cloud/view/Over2Cloud/CriticalMaster/BeforeConfigEscContact1.action?searchString="+search+"&dataFor="+$("#department").val(),
	      	    success : function(data) {
	...
	$.ajax({
	      	    type : "post",
	      	    url : "/over2cloud/view/Over2Cloud/MachineOrder/getSkillDetails.action?trashEmpId="+emp.split("kk")[0],
	      	    success : function(data) {
  ...
  //	  alert(selval);
  	...
  	$("#dpetEmpDiv").show();
  	  $.ajax({
      	    type : "post",
      	    url : "view/Over2Cloud/CriticalMaster/BeforeConfigEscContact1.action?dataFor="+selval,
      	    success : function(data) {
      	    	//alert(JSON.stringify(data));
    ...
    function definecontent(selval){
  	  ...
  	  $.ajax({
    	    type : "post",
    	    url : "view/Over2Cloud/CriticalMaster/getCriticakDetail.action",
    	    success : function(data) {
    	    	$("#wings").empty();
  ...
  /*   //appending saved emp for floor
    function appendsavedempforfloor(floorid1){
  	  $.ajax({
    	type:"post",
    	url: "/over2cloud/view/Over2Cloud/MachineOrder/Escalation_Conf/getmappedemployee.action?floorName="+floorid1+'&deptId='+$('#department').val(),
    	success: function(data){
    	$(data).each(function(index)
                	{
    	//alert(wingid1);
	...
	$("#shiftTO2").hide();
  	 shift_employeeIdsForSaveAs=[];
      	$.ajax({
      	type:"post",
      	url: "view/Over2Cloud/CriticalMaster/getmappedemployee.action?deptId="+$("#department").val()+"&shiftId="+shiftId+"&wingIds="+wing+"&viewFlag="+viewFlag,
      	async:false,
      	success: function(data){
      	//console.log(data);
  	...
  	/*  $('#'+dropid1+' ul li').each(function(i)
	{
	 ...
	 return $(n).attr('id');
  	...
  	$.ajax({
         	 type:"post",
         	 url:"/over2cloud/view/Over2Cloud/MachineOrder/Escalation_Conf/addEscContactConfig.action",
 ...
 function adddivobj(divid){
	...
	checkflag=true;
	      	...
	      	if(checkflag==true){
	      	   $('#message').empty();
	      	$("#message").fadeIn('fast');
	      	$('#message').append("Contact already mapped to this machine!!!!").css('color','red');
	...
	$("#maxmizeViewDialog").dialog('open');
	 ...
	 $.ajax({
	  type : "post",
	   	    url : "/over2cloud/view/Over2Cloud/CriticalMaster/deleteEmpByTrash.action",
	 ...
	 $.ajax({
	  type : "post",
	   	    url : "/over2cloud/view/Over2Cloud/CriticalMaster/deleteEmpByTrash.action?viewFlag=ALL",
	...
	// alert(realid);
	 ...
	 if(checkdel=='shift')
	 {
	 url1="/over2cloud/view/Over2Cloud/MachineOrder/deleteShift.action?viewFlag=empOnly";
	 }else if(checkdel=='all')
	 {
	 url1="/over2cloud/view/Over2Cloud/MachineOrder/deleteShift.action?"; 
	 }
...
function refresh()
{
	 var deptId= $("#department").val();
 	  $.ajax({
   	    type : "post",
   	    url : "/over2cloud/view/Over2Cloud/CriticalMaster/getCriticakDetail.action",
   	    async:false,
   	    success : function(data) {
   	    	$("#wings").empty();
   	    	dynamiccount=data.length;
...
function addShift()
{
	...
	$("#maximizeDataDiv").html("<br><br><br><br><br><center><img src=images/lightbox-ico-loading.gif></center>");
	 ...
	 $.ajax({
	  type : "post",
	  	    url : "/over2cloud/view/Over2Cloud/MachineOrder/saveEmpWing.action",
...
function saveAs(){
	...
	//console.log(shift_employeeIdsForSaveAs);
	 $.ajax({
	  type : "post",
  	    url : "view/Over2Cloud/CriticalMaster/saveEmpWing.action",
...
function fetchLiveShift(){
	$.ajax({
	  type : "post",
 	    url : "/over2cloud/view/Over2Cloud/MachineOrder/viewShift.action?deptId="+$("#department").val()+"&viewFlag=live",
 	    	success : function(data) {
 	    	if(data.length!=0){
 	    	$(".shiftTagL").css('display', 'inline-block');
 ...
 function saveShift(){
	...
	$.ajax({
	  type : "post",
	    url : "view/Over2Cloud/CriticalMaster/addShiftWithWingMap.action",
	    data:{shiftemployeemappedwing:$("#shiftemployeemappedwing").val(),deptId:$("#deptId").val(),shiftId:$("#shift_name1").val(),fromTime:$("#fromShift").val().trim(),toTime:$("#toShift").val().trim()},
	    async:false,
	    success : function(data) {
	    	//alert(data);
	    	 $('#message').empty();
 ...
 function ViewShif(){
	...
	$.ajax({
	  type : "post",
 	    url : "/over2cloud/view/Over2Cloud/MachineOrder/viewShift.action?deptId="+$("#department").val(),
 	    	success : function(data) {
 	    	//console.log(data);
 	    	  	//$("#catalog ul").empty();
 	    	  	var temp='';
...
function editShiftTime(){
	$("#shiftViewDiv").hide();
	 $.ajax({
	  type : "post",
	  	    url : "/over2cloud/view/Over2Cloud/MachineOrder/editShiftTime.action",
...
function save(){
	...
	//alert(shift_employeemapped_wing);
	 $.ajax({
	  type : "post",
  	    url : "view/Over2Cloud/CriticalMaster/addShiftWithWingMapSave.action",
...
<div style="display: none;" id="trashMulDiv"></div>
	...
	<div id="shiftViewDiv" style="margin: 53px 21px 16px 21px;overflow: auto;height: 400px;"></div>
    	 	...
    	 	<td valign="top">
	            ...
	            <div class="draggable-demo-title jqx-rc-t" onclick="">Critical 
