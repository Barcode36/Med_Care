...
function createTestNameMaster()
{
	...
	$.ajax({
	    ...
	    //url : "view/Over2Cloud/Critical/viewAddTestType.action",
	    url : "view/Over2Cloud/CriticalMaster/viewAddTestName.action",
...
function uploadExcel()
{
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/CriticalMaster/beforeUpload.action",
	    success : function(data) {
       		$("#data_part").html(data);
		},
	    error: function() {
...
function editRow()
{
	...
	{
		jQuery("#addTypeName").jqGrid('editGridRow', row ,{height:222, width:270,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
...
function deleteRow()
{
	  var status = jQuery("#addTypeName").jqGrid('getCell',row,'flag');
	if(row==0)
	{
	...
	{
		$("#addTypeName").jqGrid('delGridRow',row, {height:122, width:270,reloadAfterSubmit:true,closeAfterEdit:false,top:0,left:350,modal:true,caption:'Inactive',msg: "Inactive selected record(s)?",bSubmit: "Inactive" , afterSubmit: function () {
			getOnChangeMachineData('Active');
	    }});
...
function reload(rowid, result) {
	  $("#addTypeName").trigger("reloadGrid"); 
	}


function searchData()
{
	jQuery("#addTypeName").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true, cssClass:"textField"} );
...
function getOnChangeMachineData()
{
	$.ajax({
	    type : "post",
	   	...
	   	url : "view/Over2Cloud/CriticalMaster/beforeTestNameMater.action",
...
<div class="list-icon">
 <div class="head">Test Name </div><div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">
