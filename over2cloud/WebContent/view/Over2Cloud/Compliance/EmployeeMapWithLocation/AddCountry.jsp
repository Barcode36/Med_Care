...
<SCRIPT type="text/javascript">
 ...
 function beforebranchOfficeConfig()
	{
...
<div style="overflow:auto; width: 96%; margin: 1%; border: 1px solid #e4e4e5; -webkit-border-radius: 6px; -moz-border-radius: 6px; border-radius: 6px; -webkit-box-shadow: 0 1px 4px rgba(0, 0, 0, .10); -moz-box-shadow: 0 1px 4px rgba(0, 0, 0, .10); box-shadow: 0 1px 4px rgba(0, 0, 0, .10); padding: 1%;">
    <s:form id="CountryForm" name="CountryForm" action="addCountry" namespace="/view/Over2Cloud/Compliance/Location" theme="simple"  method="post" >
	  <div class="clear"></div>
		<div class="clear"></div> 
	   <center>
		     ...
		     <s:if test="%{mandatory}">
		     	<span id="complSpan" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{key}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
				    ...
				    <s:if test="%{mandatory}"><span class="needed"></span></s:if>
				         <s:textfield cssStyle="width: 92%" name="%{key}" id="%{key}"  maxlength="50" cssClass="textField" placeholder="Enter Data"/>
				    </div>
			        </div>
		  </s:iterator>
		 ...
		 <ul>
	        ...
	        <sj:submit 
     		  	  ...
     		  	  />
     		  	    <sj:a 
						button="true" href="#" 
						onclick="resetTaskType('CountryForm');resetColor('.pIds');"
						>
						Reset
					</sj:a>
     		  	  <sj:a 
						button="true" href="#" 
						onclick="beforebranchOfficeConfig();"
						>
						Back
					</sj:a>
	        </div>
