...
<script  src="js/jquery-ui.js"></script>
  ...
  //to get list of all department
    $.ajax({
    	    type : "post",
    	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/LocationList.action?deptFlag=loc",
    	    success : function(data) {
           	$('#department option').remove();
          	$('#department').append($("<option></option>").attr("value",-1).text("Select Location"));
              	$(data).each(function(index)
          	{
              	//alert(this.id);	
          	   $('#department').append($("<option></option>").attr("value",this.id).text(this.loc));
          	});
  ...
  function uploadExcel()
  {
	...
	$.ajax({
	    type : "post",
	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/beforeRosterUpload.action",
	    success : function(data) {
       	$("#data_part").html(data);
	},
	    error: function() {
  ...
  function truncate()
  {
	//alert("Hellow--");
		$.ajax({
		    type : "post",
		    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/truncketTable.action",
		    success : function(data) {
	       	$("#"+"resultTarget").html(data);
			setTimeout(function(){ $("#resultTarget").fadeIn(); }, 10);
		    setTimeout(function(){ $("#resultTarget").fadeOut(); }, 1000);
		},
		    error: function() {
	            alert("error");
	        }
...
function maptomultiplewing(emp){
	...
	$("#maxmizeViewDialog1").dialog({title:'Map '+$('#'+emp.id).text()+' to Sub Dept',height:400,width:240,dialogClass:'transparent'});
	
	$("#maximizeDataDiv1").html("<br><br><br><br><br><center><img src=images/lightbox-ico-loading.gif></center>");
	 $.ajax({
	    type : "post",
	    url : "/over2cloud/view/Over2Cloud/CoprporatePatient/Map_Resources/multi_mapping_roster.jsp",
	    success : function(data) 
	    {
...
var delay = (function(){
	...
	$.ajax({
	      	    type : "post",
	      	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getEmployeeByDept.action?searchString="+search+"&deptId="+$("#department").val(),
	      	    success : function(data) {
	...
	$.ajax({
	      	    type : "post",
	      	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getSkillDetails.action?trashEmpId="+emp.split("kk")[0],
	      	    success : function(data) {
    ...
    $("#shiftViewDiv").hide();
  	$("#dpetEmpDiv").show();
  	  $.ajax({
      	    type : "post",
      	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getEmployeeByLoc.action?deptId="+selval,
      	    success : function(data) {
      	    	//alert(JSON.stringify(data));
      	    //	console.log(data);
                     	$('#catalog ul').empty();
    ...
    //to define wings and floor according to mapping of deparment 
     function definecontent( ){
    	 wingIds='';
    	// $("#wingIds").removeAll();
    var loc=$("#department option:selected").text();
    $("#wings").html("<br><br><br><br><br><br><br><br><br><br><br><br><center><img src=images/ajax-loaderNew.gif></center>");
  	  $.ajax({
    	    type : "post",
    	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getLocationMapDetail.action?LocId="+loc,
    	    success : function(data) {
    	    	$("#wings").empty();
	...
	$("#shiftTO2").hide();
  	 shift_employeeIdsForSaveAs=[];
      	$.ajax({
      	type:"post",
      	url: "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getMappedEmployeeRoster.action?locName="+$("#department option:selected").text()+"&shiftId="+shiftId+"&wingIds="+wing+"&viewFlag="+viewFlag,
      	async:false,
      	success: function(data){
      	//console.log(data);
      ...
      if(data.length>0){	
    	  ...
    	  $(data).each(function(index){
          	...
          	shift_employeeIdsForSaveAs.push(this.id+"kk"+this.wingid);
          	//console.log("MMMMMMMMMMMMMMM"+this.id+"kk"+this.wingid);
          	test1=this.id;
          	test2=this.wingid;
    	  });
      }
 ...
 function adddivobj(divid){
	...
	checkflag=true;
	      	...
	      	if(checkflag==true){
	      	   $('#message').empty();
	      	$("#message").fadeIn('fast');
	      	$('#message').append("Contact already mapped to this sub department!!!!").css('color','red');
	...
	//console.log("name "+name+" STR "+str+" deptlevel "+deptlevel+" empIdempId "+empId+" pid "+pid+"  res  res  "+res+" floorId "+floorId+"  deptId "+deptId+"pid2 "+pid+" resresresres 2 "+res);
	...
	addFlag=true;
	      	...
	      	//console.log("check"+dropid2+"-"+dropname2+" RES "+res);
	 ...
	 function beforupdateConfirm(con){
		 ...
		 var cc=wingid.split("kk")[2];
		var daySerID= wingid.split("kk")[1]+"kk"+ wingid.split("kk")[2];
		 $("#save").attr('onclick',' updateConfirm("'+con+'")').css('opacity','1');
		 $("#maxmizeViewDialog").dialog('close');
		 $('#'+remId+"-"+daySerID).fadeOut(200);
		   	 $('#'+remId+"-"+daySerID).remove();
		   	 $("#saveAs").attr('onclick',' saveAs()').css('opacity','1');
		   	if(con=='yes'){
		   	
		   	 var index = shift_employeemapped_wing.indexOf(remId+'#'+cc);
		 	if (index > -1) {
		 	shift_employeemapped_wing.splice(index, 1);
		 	}
		
		 	 var index = shift_employeeIdsForSaveAs.indexOf(remId+'#'+daySerID);
		 	if (index > -1) {
		 	shift_employeeIdsForSaveAs.splice(index, 1);
			   	...
			   	{
			   	removeFromSaveAs.push(remId+'#'+shift_employeeIdsForSaveAs[j].split("kk")[1]+'kk'+shift_employeeIdsForSaveAs[j].split("kk")[2]);
			   	//console.log(remId+"vv "+shift_employeeIdsForSaveAs[j].split("kk")[1]+'kk'+shift_employeeIdsForSaveAs[j].split("kk")[2]);
			   	 if(remId==shift_employeeIdsForSaveAs[j].split("kk")[0])
			   	 {
			   		//console.log(remId+" ---- "+shift_employeeIdsForSaveAs[j].split("kk")[0]);
			   	 $('#'+remId+"-"+shift_employeeIdsForSaveAs[j].split("kk")[1]+'kk'+shift_employeeIdsForSaveAs[j].split("kk")[2]).fadeOut(200);
			   //	console.log(remId+" --1-- "+shift_employeeIdsForSaveAs[j].split("kk")[1]+'kk'+shift_employeeIdsForSaveAs[j].split("kk")[2]);
			   	   	 $('#'+remId+"-"+shift_employeeIdsForSaveAs[j].split("kk")[1]+'kk'+shift_employeeIdsForSaveAs[j].split("kk")[2]).remove();
			   	 }
			   	}
	 ...
	 $.ajax({
	  type : "post",
	   	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/deleteEmployee.action",
	 ...
	 $.ajax({
	  type : "post",
	   	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/deleteEmpAllService.action?viewFlag=ALL",
	   	    data :  { trashEmpId: $("#trashMulDiv").text(),wingIds:wingid,shiftId:shiftid },
	   	    success : function(data) {
	 ...
	 myFunction($("#"+remId+"-"+wingid).text(),remId,realid);
	}
	 ...
	 if(checkdel=='shift')
	 {
	 url1="/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/deleteShift.action?viewFlag=empOnly";
	 }else if(checkdel=='all')
	 {
	 url1="/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/deleteShift.action?"; 
	 }
...
function refresh()
{
	var dept=$("#department option:selected").text();
	 var deptId= $("#department").val();
 	  $.ajax({
   	    type : "post",
   	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/getDepartmentMapDetail.action?deptId="+dept,
   	    async:false,
   	    success : function(data) {
   	    	$("#wings").empty();
   	    	dynamiccount=data.length;
...
function addShift()
{
	...
	$("#maximizeDataDiv").html("<br><br><br><br><br><center><img src=images/lightbox-ico-loading.gif></center>");
	 ...
	 $.ajax({
	  type : "post",
	  	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/saveEmpShiftMapping.action",
	  	    data:{shiftEmpMapSubdept:shift_employeemapped_wing.toString(),deptId:deptId},
	  	    	success : function(data) {
...
function saveAs(){
	...
	//console.log(shift_employeeIdsForSaveAs);
	 $.ajax({
	  type : "post",
  	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/saveEmpShiftMapping.action",
  	    data:{shiftEmpMapSubdept:shift_employeeIdsForSaveAs.toString(),deptId:$("#department").val()},
  	    	success : function(data) {
  	    	//alert(data);
  	    	  	$("#maximizeDataDiv").empty();
  	    	  	$("#maximizeDataDiv").html(data);
...
function fetchLiveShift(){
	$.ajax({
	  type : "post",
 	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/viewShift.action?deptId="+$("#department").val()+"&viewFlag=live",
 	    	success : function(data) {
 	    	if(data.length!=0){
 	    	$(".shiftTagL").css('display', 'inline-block');
 ...
 function saveShift(){
	...
	$.ajax({
	  type : "post",
	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/addShiftWithSubdeptMap.action",
	    data:{shiftEmpMapSubdept:$("#shiftemployeemappedwing").val(),deptId:$("#deptId").val(),shift_name:$("#shift_name1").val(),fromShift:$("#fromShift").val().trim(),toShift:$("#toShift").val().trim()},
	    async:false,
	    success : function(data) {
	    	//alert(data);
	    	 $('#message').empty();
 ...
 function ViewShif(){
	...
	$.ajax({
	  type : "post",
 	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/viewShift.action?deptId="+$("#department").val(),
 	    	success : function(data) {
 	    	//console.log(data);
 	    	  	//$("#catalog ul").empty();
 	    	  	var temp='';
...
//	alert(shiftId);
	...
	//console.log(shift_employeeIdsForSaveAs);
	 $.ajax({
	  type : "post",
  	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/saveEmpSubdept.action",
  	    data:{viewFlag:"F"},
  	    	success : function(data) {
  	    	//alert(data);
...
function editShiftTime(){
	$("#shiftViewDiv").hide();
	 $.ajax({
	  type : "post",
	  	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/editShiftTime.action",
...
function save(){
	...
	//alert("hii");
	 $.ajax({
	  type : "post",
  	    url : "/over2cloud/view/Over2Cloud/CorporatePatientServices/cpservices/addRosterEmpMapSave.action",
  	    data :  { shiftID: $("#department").val(),shiftEmpMapSubdept:shift_employeemapped_wing.toString() },
  	    success : function(data) {
  	 $('#message').empty();
   	    	$('#message').append(data).css('color','black').css('display','block');
   	    $("#message").fadeIn('fast');
...
<div class="list-icon">
	 <div class="head">Roster</div><div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">Map</div> 
<a href="#" style="    float: right;margin-right: 60px;" onclick="uploadExcel();">Roaster Upload</a>
<a href="#" style="    float: right;margin-right: 18px;"  onclick="truncate();">Delete Roaster</a>
			<center><div id="resultTarget" style="    float: right;"></div></center>
...
<div style="display: none;" id="trashMulDiv"></div>
	...
	<div id="shiftViewDiv" style="margin: 53px 21px 16px 21px;overflow: auto;height: 400px;"></div>
    	 	...
    	 	<td valign="top">
	            ...
	            <div class="draggable-demo-title jqx-rc-t" onclick="">CPS 
	         <%--     <sj:div cssClass="button"  cssStyle="float:left" href="#" onclick=" dragdrop('catalog');" title="Add Employee" bindOn="button">ADD</sj:div>
	            ...
	            <button id="ddShift"  onclick="addShift()"  title="Add shift" style="float: left;"><img alt="Refresh_image" src="images/add.png" width="15px" height="15px"/></button>
