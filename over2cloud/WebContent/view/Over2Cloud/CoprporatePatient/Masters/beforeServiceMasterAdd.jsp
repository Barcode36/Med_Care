...
<script type="text/javascript">
	...
	function reset(formId) {
	 ...
	 { 
		 var divId="service_name";
	 	$.ajax({
	 	    type : "post",
	 	    async:false,
	 	    url : "view/Over2Cloud/CorporatePatientServices/cpservices/fetchServices.action",
	 	    success : function(data) {
	 	   data = data.jsonArray;
	 	  
	 	    if(data != null){
	 		    $('#'+divId+' option').remove();
	 		   $('#'+divId).append($("<option></option>").attr("value",-1).text('Select Services'));
	 ...
	 fetchService();
	 function getServices(sourceId,showID, hideID)
	 {
	   if(sourceId == 'New')
	   {
	     document.getElementById("service_nameId").value = "";
	     document.getElementById("service_name").value = "";
	     $("#"+showID).show();
	     $("#"+hideID).hide();
	     $('#'+'servicename').html("");
	     $('#'+'servicenameID').html("service_nameId#Service#T#a,");
	    }
	   else
	   {
	 	  $('#'+'servicenameID').html("");
	 	 $('#'+'servicename').html("service_name#Service#D#D,");
	 }
  }
	 function callDDService(value,showID, hideID)
	 {
	 	if(value=='')
	 	{
	 	 $("#"+showID).hide();
	 	var cc=$('#'+'servicenameID').html("");
	 	//alert(cc);
	 	$('#'+'servicenameID').html("");
	    $("#"+hideID).show();
	    $('#'+'servicename').html("service_name#Service#D#D,");
...
<body>
	...
	<div class="middle-content">
		...
		<div class="list-icon">
			<div class="head">Service Master Details</div>
			<div class="head">
			...
			<div class="container_block">
				<div style="float: left; padding: 20px 1%; width: 98%;">
					<s:form id="formone" name="formone"
						...
						action="addServiceMasterDetails" theme="css_xhtml" method="post"
		     	...
		     	<span id="complSpan" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
				    ...
				    <s:if test="%{mandatory}"><span class="needed"></span></s:if>
				       ...
				       <!--   <s:else>
				         ...
				         <sj:a value="+" onclick="adddiv('100')" button="true" cssClass="submit" cssStyle=" margin-left: 270px; margin-top: -30px;">+</sj:a>
				         </s:else> -->
						...
						<!-- 
						<div id="extraDiv">
                 <s:iterator begin="100" end="110" var="typeIndx">
                        <div class="clear"></div>
                         <div id="<s:property value="%{#typeIndx}"/>" style="display: none">
                             <s:iterator value="packageFields" status="counter" begin="1">
                            <div class="newColumn">
                                ...
                                <div class="rightColumn1">
                                    <s:textfield name="service_name"  id="service_name%{#typeIndx}"  cssClass="textField" cssStyle="margin:0px 0px 10px 0px;" placeholder="Enter Data" />
                                    </div>
				  	...
				  	<span id="servicename" class="pIds" style="display: none; ">service_name#Service#D#D,</span>
					 <span id="servicenameID" class="pIds" style="display: none; ">service_nameId#Service#T#sc,</span>
				         ...
				         <div id="patient_nameDD">
                                      ...
                                      cssStyle="width:75%"
                                      theme="simple"
                                      onchange="getServices(this.value,'patient_nameText','patient_nameDD');"
                                      >
                          </s:select>
							</div>
					    	<div id="patient_nameText" style="display: none">
					    		<s:textfield id="service_nameId" name="service_name"   theme="simple" cssClass="textField" placeHolder="Enter Patient Name" onblur="callDDService(this.value, 'patient_nameText','patient_nameDD');"/>
					    	</div>
				    </div>
			        </div>
