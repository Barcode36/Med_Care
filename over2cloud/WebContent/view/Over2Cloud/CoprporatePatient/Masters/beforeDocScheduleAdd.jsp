...
<script type="text/javascript">
	...
	function getServiceList(state,div){
		//alert(state);
		$.ajax({
		    type : "post",
		    url : "view/Over2Cloud/CorporatePatientServices/cpservices/fetchSerivceList.action?searchField="+state,
	...
	function fetchSpecList(state,div){
	//	alert(div);
		$.ajax({
		    type : "post",
		    url : "view/Over2Cloud/CorporatePatientServices/cpservices/fetchSpecList.action?searchField="+state,
		    success : function(data) {
		    	$('#'+div+' option').remove();
		$('#'+div).append($("<option></option>").attr("value",-1).text('--Select Speciality Name--'));
		$(data).each(function(index)
		{
			$('#'+div).append($("<option></option>").attr("value",this.id).text(this.name));
		});
...
<body>
	...
	<div class="middle-content">
		...
		<div class="list-icon">
			<div class="head">Doctor Schedule</div>
			<div class="head">
			...
			<div class="container_block">
				<div style="float: left; padding: 20px 1%; width: 98%;">
					<s:form id="formone" name="formone"
						...
						action="addDocScheduleDetails" theme="css_xhtml" method="post"
				...
				<s:iterator value="packageFields" status="status">
		     	...
		     	<span id="complSpan" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
				    ...
				    <s:if test="%{mandatory}"><span class="needed"></span></s:if>
				         ...
				         <s:if test="%{key == 'sch_loc'}">
				         		<s:select 
                                      id="%{key}"
                                      name="%{key}" 
                                      list="{'Medanta The Medicity','Medanta Mediclinic','Cybercity Medanta'}"
                                      headerKey="-1"
                                      headerValue="--Select Location--"
                                      cssClass="select"
                                      cssStyle="width:82%"
                                      theme="simple"
                                      onchange="getServiceList(this.value,'sch_service'); fetchSpecList(this.value,'sch_spec')"
                                      >
                          </s:select>
				         
				         </s:if>
				             <s:elseif test="%{key == 'sch_service'}">
				         		<s:select 
                                      id="%{key}"
                                      name="%{key}" 
                                      list="{'No Data'}"
				         ...
				         <s:elseif test="%{key == 'sch_ftime'}">
				      		 <sj:datepicker name="%{key}"  id="%{key}" placeholder="Enter Time" cssStyle="margin:0px 0px 10px 0px"  cssClass="textField" timepickerOnly="true" timepicker="true" timepickerAmPm="false"  size="20"   readonly="true"   showOn="focus"/>
				         <sj:a value="+" onclick="adddiv('100')" button="true" cssClass="submit" cssStyle=" margin-left: 352px; margin-top: -30px;">+</sj:a>
				         </s:elseif>
				          <s:elseif test="%{key == 'sch_ttime'}">
				       			 <sj:datepicker name="%{key}"  id="%{key}" placeholder="Enter Time" cssStyle="margin:0px 0px 10px 0px"  cssClass="textField" timepickerOnly="true" timepicker="true" timepickerAmPm="false"  size="20"   readonly="true"   showOn="focus"/>
				         </s:elseif>
				         <s:else>
    ...
    padding: 1%; overflow: auto; "  class="container_block" >
		     	...
		     	<span id="complSpan" class="pIds" style="display: none; "><s:property value="%{key}"/><s:property value="%{#typeIndx}" />#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
				          ...
				          <s:if test="%{key == 'sch_ttime'}">
				         <sj:datepicker name="%{key}" id="%{key}%{#typeIndx}" placeholder="Enter Time" cssStyle="margin:0px 0px 10px 0px"  cssClass="textField" timepickerOnly="true" timepicker="true" timepickerAmPm="false"  size="20"   readonly="true"   showOn="focus"/>
