...
function addNew(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/CorporatePatientServices/cpservices/beforeAddServiceMaster.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/CorporatePatientServices/cpservices/viewServiceMasterDetails.action?status="+$("#flag").val()+"&serviceID="+$("#serviceID").val(),
...
var selRowIds = jQuery('#gridServiceMaster').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
			jQuery("#gridServiceMaster").jqGrid('editGridRow', row ,{reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true});
			reloadPage();
...
function deleteRow()
{
	var test=jQuery("#gridServiceMaster").jqGrid('getCell',row,'serv_loc');
	var serviceID=jQuery("#gridServiceMaster").jqGrid('getCell',row,'service_name');
	//alert(test+" serviceID "+serviceID);
var status = jQuery("#gridServiceMaster").jqGrid('getCell',row,'status');
if(row==0)
{
...
else
{
	...
	//$("#gridServiceMaster").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:true,top:0,left:350,modal:true});  
$.ajax(
			{
				type : "post",
				url :"view/Over2Cloud/CorporatePatientServices/cpservices/deleteLocation.action?id="+serviceID+"&location_name="+test,
			    success : function(data) 
			    {
				reloadPage();
			    },
...
function searchRow()
{
	 $("#gridServiceMaster").jqGrid('searchGrid',row, {height:120,reloadAfterSubmit:true,top:0,left:350,modal:true});
}
 
function reloadPage()
{
var grid = $("#gridServiceMaster");
