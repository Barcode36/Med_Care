...
<script type="text/javascript">
	...
	$.ajax({
				type : "post",
				url : "/over2cloud/view/Over2Cloud/HelpDeskOver2Cloud/Escalation_Conf/getfloorwingsdetail.action?deptId="
						+ $('#department').val()
						+ "&subDeptId="
						+ $('#subdepartment').val(),
				success : function(data) {
					...
					/* 	if(data[0].flag=='wing'){
					    	...
					    	$(data).each(function(index)
					        		{
					    			if(ct==0){
					            		$("#main_container").append('<div style="width:98%; background:-webkit-gradient(linear, 0 0, 0 100%, from(#f8f8f8), to(#eaeaea));">'+this.floorname+'</div>');
					            		$("#main_container").append('<hr/>');
					            		ct++;
					            	}else {
					            		if(data[(index-1)].floorname===data[index].floorname){
					            		}else{
					            			$("#main_container").append('</br><div style="width:98%; background:-webkit-gradient(linear, 0 0, 0 100%, from(#f8f8f8), to(#eaeaea));">'+this.floorname+'</div>');
					            			$("#main_container").append('<hr/>');
					            		}
					            	}
					    		
					            	$("#main_container").append('<div style=""><input id="'+this.wingid+'kk'+this.floorid+'chkbox" type="checkbox"  style="margin-top:5px;" value="'+this.wingid+'">'+this.wingname+'</div>');
					              	var wingsplittedids=wingsidsforinput.split(',');
					            	for(var i=0;i<wingsplittedids.length;i++){
					    				if(wingsplittedids[i].length>0){
					    					if(wingsplittedids[i]==this.wingid){
					    						 $("#"+this.wingid+'kk'+this.floorid+'chkbox').attr("disabled", true);
					    					}
					    				}
					    			}
							 ...
							 $(data).each(function(index)
					        		{
					                      	$("#main_container").append('<div style="border: 1px solid #A8A8A8;border-radius: 3px;background: #F4F4E8;"><input id="'+this.floorid+'chkboxflr" type="checkbox" style="margin-top:0px;border: 1px solid #c26" value="'+this.floorid+'">'+this.floorname+'</div>');
					                      	$("#main_container").append('<hr/>');
					                      	var floorplittedids=wingsidsforinput.split(',');
					                      	for(var i=0;i<floorplittedids.length;i++){
					        	    				if(floorplittedids[i].length>0){
					        	    					if(floorplittedids[i]==this.floorid){
					        	    						 $("#"+this.floorid+'chkboxflr').attr("disabled", true);
					        	    					}
					        	    				}
					        	    			}
					        		}); 
	...
	function saveEmployee() {
		...
		//	console.log(checkedWingIds.toString());
						/* $.ajax({
					  	    ...
					  	    data :  { shiftID: $("#shiftId1").val(),mployeemappedwing:checkedWingIds.toString(),empId:empid },
					  	    async:false,
					  	    success : function(data) {
					  		 $('#message').empty();
