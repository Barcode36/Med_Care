...
$.subscribe('getStatusForm', function(event,data)
  {
	  ...
	  var feedCC = jQuery("#gridedittable").jqGrid('getCell',feedid,'feed_cc');
	  var feedType = jQuery("#gridedittable").jqGrid('getCell',feedid,'feedtype');
	  var feedCategory = jQuery("#gridedittable").jqGrid('getCell',feedid,'feedback_catg');
	  var feedby   = jQuery("#gridedittable").jqGrid('getCell',feedid,'clientFor');
	  var feedmob  = jQuery("#gridedittable").jqGrid('getCell',feedid,'clientName');
	  var feedemail= jQuery("#gridedittable").jqGrid('getCell',feedid,'offeringName');
	  ...
	  if (feedStatus=='Pending') {
		  $("#data_part").load(connection+"/view/Over2Cloud/HelpDeskOver2Cloud/Lodge_Complaint/feedAction.action?feedStatus="+feedStatus.split(" ").join("%20")+"&feedId="+feedid.split(" ").join("%20")+"&ticketNo="+ticketNo.split(" ").join("%20")+"&feedbackBy="+feedby.split(" ").join("%20")+"&mobileno="+feedmob.split(" ").join("%20")+"&emailId="+feedemail.split(" ").join("%20")+"&feedBreif="+feedbrief.split(" ").join("%20")+"&openDate="+opendate.split(" ").join("%20")+"&openTime="+opentime.split(" ").join("%20")+"&subCatg="+subcatg.split(" ").join("%20")+"&allotto="+allotto.split(" ").join("%20")+"&feedBY="+feedBY.split(" ").join("%20")+"&feedCC="+feedCC.split(" ").join("%20")+"&feedType="+feedType.split(" ").join("%20")+"&feedCategory="+feedCategory.split(" ").join("%20")+"&moduleName="+module);
	  }
...
$.subscribe('getMoreData', function(event,data)
{
 	 ...
 	 else 
	 {
		...
		/*  $("#gridedittable").jqGrid('setGridParam', 
		 {
		  	url:"view/Over2Cloud/HelpDeskOver2Cloud/Lodge_Complaint/viewMoreFeedbackDetail.action?feedStatus="+feedStatus+"&fromDate="+fromDate+"&toDate="+toDate+"&dataFlag=M"
			,datatype:'JSON'
		}).trigger("reloadGrid"); */
	 }
});
...
<s:hidden id="moduleName" value="%{moduleName}"></s:hidden>
