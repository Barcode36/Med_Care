...
function getDepartment(module){
	  $.ajax({
	    type : "post",
	    url : "/over2cloud/view/Over2Cloud/HelpDeskOver2Cloud/Escalation_Conf/departmentList.action?module="+module,
	    success : function(data) {
	    	console.log(data);
     		$('#escDept option').remove();
    		$('#escDept').append($("<option></option>").attr("value","All").text("Select Department"));
        	$(data).each(function(index)
    		{
        	//alert(this.id);	
    		   $('#escDept').append($("<option></option>").attr("value",this.id).text(this.dept));
    		});
				...
				<div class="newColumn">
        			   <div class="leftColumn" >Module:&nbsp;</div>
					        <div class="rightColumn">
				                  <s:select  id="module"
				                              name="module"
				                              list="{'ORD','CRF','Emergency','Pharmacy','LabOrd'}"
				                              headerKey="-1"
