...
public class HISLabOrderFetch  extends Thread  {
	...
	private LabOrderFetchHelper CDIH =  new LabOrderFetchHelper();
	...
	public HISLabOrderFetch(SessionFactory connection, Session sessionHis ) {
				...
				try{
					while(true){
						...
						//t2mvirtual.getVirtualNoData(connection);
							...
							CDIH.DATAFetchLabIP(log, connection, sessionHis, ch);
							CDIH.DATAFetchLabOP(log, connection, sessionHis, ch);
							//CDIH.DATAFetchLabUpdate(log, connection, sessionHis, ch);
	...
	public static void main(String[] args) {
		...
		try{
			...
			System.out.println("After HIS DB connection"+sessionHis);
				...
				Thread uclient=new Thread(new HISLabOrderFetch(connection, sessionHis));
