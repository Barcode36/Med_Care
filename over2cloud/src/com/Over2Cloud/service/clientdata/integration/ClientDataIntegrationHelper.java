...
System.out.println("This is new file >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		try
		{
			...
			rs = st.executeQuery("select * from Dreamsol_Mc_ord_VW where ORD_DATE_TIME > TO_TIMESTAMP('" + lastORDDateTime + "','YYYY-MM-DD HH24:MI:SS.FF') AND ORDER_TYPE_CODE IN('MXR','MUSG', 'MUCT','HSME')");//15-12-2015 aarif
			...
			while (rs.next())
			{
				...
				// System.out.println("CAN_DATE_TIME : "+rs.getString("CAN_DATE_TIME").toString());
				// System.out.println("CAN_LINE_REASON : "+rs.getString("CAN_LINE_REASON").toString());
				...
				if (rs.getString("ORDER_ID") != null && !rs.getString("ORDER_ID").toString().equals(""))
				{
					...
					// System.out.println("ordid : "+rs.getString("ORDER_ID").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("ORDER_TYPE_CODE") != null && !rs.getString("ORDER_TYPE_CODE").toString().equals(""))
				{
					...
					if (rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("MUCT"))
					{
				...
				if (rs.getString("PATIENT_NAME") != null && !rs.getString("PATIENT_NAME").toString().equals(""))
				{
					...
					// System.out.println("pName : "+rs.getString("PATIENT_NAME").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("BED_NUM") != null && !rs.getString("BED_NUM").toString().equals(""))
				{
					...
					// System.out.println("roomNo : "+rs.getString("BED_NUM").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("ADDED_BY_ID") != null && !rs.getString("ADDED_BY_ID").toString().equals(""))
				{
					...
					// System.out.println("orderBy : "+rs.getString("ADDED_BY_ID").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("ORD_DATE_TIME") != null && !rs.getString("ORD_DATE_TIME").toString().equals(""))
				{
					...
					// System.out.println("orderDate : "+rs.getString("ORD_DATE_TIME").toString().split(" ")[0]);
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("ORD_DATE_TIME") != null && !rs.getString("ORD_DATE_TIME").toString().equals(""))
				{
					...
					// System.out.println("orderTime : "+rs.getString("ORD_DATE_TIME").toString().split(" ")[1]);
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("PRACTITIONER_NAME") != null && !rs.getString("PRACTITIONER_NAME").toString().equals(""))
				{
					...
					// System.out.println("treatingDoc : "+rs.getString("PRACTITIONER_NAME").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				if (rs.getString("DOCTOR_SPECIALITY") != null && !rs.getString("DOCTOR_SPECIALITY").toString().equals(""))
				{
					...
					//System.out.println("treatingSpec : " + rs.getString("DOCTOR_SPECIALITY").toString());
				} else
				{
					ob.setDataName("NA");
				}
				...
				try
				{
					...
					// here department is hard coded from occ sub department
					// coz. have to start escalation to occ level..
					ob = new InsertDataTable();
				...
				//System.out.println("chkRepeateOrd " + chkRepeateOrd);
				//System.out.println("ordNameChange >>> " + ordNameChange);
				//System.out.println("jshgfjsf >>> " + rs.getString("ORDER_TYPE_CODE").toString());
				if (chkRepeateOrd == true)
				{
					if (rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("CARD") || rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("MUSG") || rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("MXR") ||  rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("MUCT")||  rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("HSME"))//15-12-2015 by aarif
					{
						//System.out.println("sts :>>>>>>>>>>>. " + ordNameChange);
			...
			rs = st.executeQuery("select * from Dreamsol_Mc_ord_VW where TRUNC (CAN_DATE_TIME) = TRUNC (SYSDATE) AND ORDER_TYPE_CODE IN('MXR','MUSG','MUCT','HSME')  ");  	//15-12-2015  by aarif
				...
				if (chkFlag)
				{
					...
					if (chk > 0)
					{
						...
						if (temp != null && !temp.isEmpty())
						{
							...
							else if (rs.getString("ORDER_TYPE_CODE").toString().equalsIgnoreCase("MUCT") && i)
							{
								String namemob = getAllotToOfCancel(rs.getString("ORDER_ID"), clientConnection);
								if (!namemob.equalsIgnoreCase(""))
								{
		...
		} catch (Exception e)
		{
			...
			if(e.toString().contains("ORA"))
			{
				System.out.println("Redirecting to New Server!!!");
				ClientDataIntegrationRedirectServer CDR = new ClientDataIntegrationRedirectServer();
				CDR.datafetchForOrder(log, clientConnection, sessionHis, ch);
				//CDR.datafetchForOrderEM(log, clientConnection, sessionHis, ch);
			}
	...
	public void datafetchForOrderEM(Log log, SessionFactory clientConnection, Session sessionHis, CommunicationHelper ch)
	{
		...
		try
		{
            ...
            System.out.println("Emergency patient Fetch1111111111111>>>>>>>>>>>>>>>");
            ...
            {
                insertData.clear();
                System.out.println("Emergency patient Fetch2222222222222222222222>>>>>>>>>>>>>>");
...
") AND ORDER_TYPE_CODE IN('MXR','MUSG', 'MUCT','HSME')  "
        ...
        } catch (Exception e)
		{
			...
			e.printStackTrace();
			log.info("Error in ClientDataIntegrationHelper.java \n " + e.getStackTrace());
			ch.addMessage("Manab", "Alert", "8882572103", "EM Order fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			ch.addMessage("Chandrabhan", "Alert", "8826336111", "EMOrder fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			ch.addMessage("Prabhat", "Alert", "9250400315", " EM Order fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			ch.addMessage("Sanjay", "Alert", "8130772488", " EM Order fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			ch.addMessage("Kamlesh", "Alert", "8010797571", " EM Order fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			//ch.addMessage("Abhay", "Alert", "9873325443", "EM Order fetche File IPD 88 Server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			//ch.addMessage("Manab", "Alert", "8882572103", "Order fetche File IPD  88 server: "+e.toString().substring(0, 30) , "", "Pending", "0", "ORD", clientConnection);
			if(e.toString().contains("ORA"))
			{
				System.out.println("Redirecting to New Server!!!");
	...
	private String  machineID(String orderType, SessionFactory connection)
	{
		...
		try
		{
			...
			//15-12-2015 by aarif
			if("ECHO".equalsIgnoreCase(orderType)){
				machineName="Echo";
			}
